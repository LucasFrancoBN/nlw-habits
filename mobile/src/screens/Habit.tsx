...
export function Habit() {
  ...
  const habitProgress = dayInfo?.possiblesHabits.length
    ? generateProgressPercentage(
        dayInfo.possiblesHabits.length,
  ...
  return (
    <View className="flex-1 bg-background px-8 pt-16">
      ...
      >
        ...
        >
          {dayInfo!.possiblesHabits.length > 0 ? (
            dayInfo!.possiblesHabits.map((habit) => (
              <Checkbox
                key={habit.id}
                title={habit.title}
        ...
        {isDateInPast && (
          <Text className="text-white mt-10 opacity-60">
